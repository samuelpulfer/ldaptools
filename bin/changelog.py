...
fin = os.popen('git log --summary --stat --no-merges --date=iso8601', 'r')
...
for line in fin:
    ...
    # Match the date line
        ...
        date_short = re.split('\+', date, 1)
        #print date_short[0]
        date_object = datetime.strptime(date_short[0].strip(), '%Y-%m-%d %H:%M:%S')
        dateFound = True
    ...
    if authorFound & dateFound & messageFound & filesFound:
        ...
        # author on this day
        authorLine = date_object.strftime(date_format) + "  " + author
